...
class AI2ThorEnv(gym.Env):
    ...
    def __init__(self, seed=None, config_file='config_files/config_example.json', config_dict=None):
        ...
        self.metadata_last_object_attributes = ['lastObjectPut', 'lastObjectPutReceptacle',
                                                ...
                                                'lastObjectClosed','lastObjectDropped']
    ...
    def step(self, action, verbose=True):
        ...
        if action_str.endswith('Object'):  # All interactions end with 'Object'
            ...
            if action_str.startswith('Put'):
                ...
                if self.event.metadata['inventoryObjects']:
                    self.event.metadata['lastObjectDropped'] =self.event.metadata['inventoryObjects'][0]
        ...
        elif action_str.startswith('Rotate') and ('Hand' not in action_str):
            if self.continuous_movement:
                # Rotate action
        ...
        elif (action_str.startswith('Move')) and ('Hand' not in action_str):
            # Move and Look actions
            self.event = self.controller.step(dict(action=action_str,moveMagnitude=0.25))
            
        elif (action_str.startswith('Look')) and ('Hand' not in action_str):
            # Move and Look actions
            self.event = self.controller.step(dict(action=action_str, moveMagnitude=0.1))
            
            
        elif action_str.startswith('MoveHand'):
            self.event = self.controller.step(dict(action=action_str, moveMagnitude=0.1))
            
        
                    
        elif action_str.startswith('Crouch'):        
            self.event = self.controller.step(
                                dict(action='Crouch'))
            
        elif action_str.startswith('Stand'):        
            self.event = self.controller.step(
                                dict(action='Stand'))
        
        elif action_str.startswith('RotateHand'):
            anglehandX = 0.0
            anglehandY = 0.0
            anglehandZ = 0.0
            if action_str.endswith('X'): 
                anglehandX=anglehandX+30.0
                self.event = self.controller.step(dict(action='RotateHand', x=anglehandX))
            if action_str.endswith('Y'): 
                anglehandY=anglehandY+30.0
                self.event = self.controller.step(dict(action='RotateHand', y=anglehandY))
            if action_str.endswith('Z'): 
                anglehandZ=anglehandZ+30.0
                self.event = self.controller.step(dict(action='RotateHand', z=anglehandZ))
        ...
        with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/file.pickle','rb') as f:
            file=pickle.load(f)
            
        try:
            with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/metadata/'+file.replace('steps.json', '')+'metadata.pickle', 'rb') as f:
                metaa = pickle.load(f)
        except:
            pass
        ...
        with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/metadata/'+file.replace('steps.json', '')+'metadata.pickle', 'wb') as f:
            metaa.append(self.event.metadata)
            # pickle.dump(metaa, f)
