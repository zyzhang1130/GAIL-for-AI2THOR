...
parser.add_argument('--max-num-steps', type=int, default=int(1000), metavar='STEPS',
...
if __name__ == '__main__':
    ...
    for mem_steps in range(args.evaluation_size):
        if done:
            ...
            file='Hide the egggg_FloorPlan12steps.json'
            with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/hide_processed'+'/'+file) as f:
                steplist = json.load(f)
                # env.controller.reset(scene=file.split('_', 3)[1].split('.', 3)[0].split('steps', 3)[0])

            with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/hide_processed'+'/'+file.replace('steps.json', 'objects.json')) as f:
                objects = json.load(f)
                objects = list(dict.fromkeys(objects))
                object_poses = []
                for i in objects:
                    object_poses.append({
                        "objectName": i.split('|', 1)[0]+'_19',
                        "rotation": {"y": 0, "x": 0, "z": 0},
                        "position": {"y": i.split('|', 3)[2], "x": i.split('|', 3)[1], "z": i.split('|', 3)[3]}
                        })
                # env.event = env.controller.step(action='SetObjectPoses', objectPoses=object_poses)
                
            with open('/home/user/Documents/Zeyu/cups-rl2_metadata_collection (manual)/file.pickle','wb') as f:
                pickle.dump(file,f)
            for i in steplist:
                  env.step(i)
